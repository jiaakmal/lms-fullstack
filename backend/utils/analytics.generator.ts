...
export async function generateLastMonthData<T extends Document>(model:Model<T>) :Promise<{last12Months:MonthData[]}>{
    ...
    for(let i =11; i>=0; i--){
        ...
        const monthYear =  endDate.toLocaleString('default',{month:'long',year:'numeric'});
